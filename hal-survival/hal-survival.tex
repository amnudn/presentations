...
\begin{frame}[label={sec:orgebfac2d}]{Multivariate càdlàg functions}
...
\begin{frame}[label={sec:org7ee2c55}]{The sectional variation norm of smooth functions}
...
\begin{frame}[label={sec:org17be9fa}]{Implementation of the estimator}
...
\begin{frame}[label={sec:org2f9023c}]{Basis functions for the HAL estimator}
\begin{onlyenv}<1>
\begin{center}
...
\label{sec:org2a86044}
\begin{frame}[label={sec:orgaa118a4}]{The solution to the minimization problem}
\small \vspace{-.5cm}
\begin{equation*}
...
\begin{frame}[label={sec:orga802f4b}]{Not clear same trick works when \(d>1\) \pause -- \color{red} don't trust \(d=1\)!}
\begin{onlyenv}<3>
...
\begin{frame}[label={sec:org576ed1b}]{The estimator \(\hat{f}_n\) in the survival setting}
\pause
...
\begin{frame}[label={sec:org9e3214e}]{Proof by picture}
\begin{equation*}
...
\label{sec:org9ce5f5f}
\begin{frame}[label={sec:org6354a11}]{\color{white} break title}
...
\begin{frame}[label={sec:org1a207fb}]{Deriving convergence rates using empirical processes theory}
\pause \small
...
\begin{frame}[label={sec:orgff0e2aa}]{This holds for \large \(f_{\hat{\beta}_n}\): \(\empmeas{[L(f_{\hat{\beta}_n}, \blank)]} \leq \empmeas{[L(f_0, \blank)]}  + \mathcal{O}_P{(r_n^2)}\)}
\small\pause
Write
\begin{equation*}
  f_0(x) = f_0(0)
...
\begin{frame}[label={sec:orgdf50c20}]{Conclusion and discussion}
\begin{itemize}[<+->]
...
\section*{References}
\label{sec:orgd803441}
\begin{frame}[label={sec:org191d228}]{References}
