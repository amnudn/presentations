...
\begin{frame}[label={sec:orgeabe21f}]{Targeted or debiased machine learning\footnote{\cite{van2011targeted,chernozhukov2018double}}}
\vspace{.3cm}
...
\begin{frame}[label={sec:org7f38ce5}]{Examples of estimands}
  ...
  \only<2>{
    \begin{beamercolorbox}[rounded=true]{gray}
      \begin{equation*}
        ...
        \E_P{
          \left[
            \prodi_0^t\left( 1 - \Lambda_P(\diff s \mid X) \right)
            % e^{-\Lambda_P(t \mid X)}
          \right]},
        \quad (T,X) \sim Q,
      \end{equation*}
      ...
      \begin{equation*}
        ...
        \E_{P}{
          \left[
            \int_0^t
            \prodi_0^u\left( 1 -
              [\Lambda_{1,P}+\Lambda_{2,P}](\diff s \mid a, W) \right)
            % e^{-[\Lambda_{1,P}+\Lambda_{2,P}](u \mid a, W)}
            \Lambda_{1,P}(\diff u \mid a, W)
          \right]},
      \end{equation*}
      % where
      \begin{equation*}
...
\begin{frame}[label={sec:orgdd198a9}]{Targeted learning with super learning}
\begin{equation*}
  \Psi(P) =
  \E_{P}{
          \left[
...
\begin{frame}[label={sec:org651c276}]{Super learning\footnote{\cite{stone1974cross,geisser1975predictive,wolpert1992stacked,breiman1996stacked,van2007super}}}
\begin{onlyenv}<1>
...
\begin{frame}[label={sec:orgebe2da2}]{\color{white} breaker}
\begin{beamercolorbox}[rounded=true]{gray}
\Large
...
\begin{frame}[label={sec:org439e673}]{The negative log-likelihood is unsuited for super learning}
\begin{center}
\includegraphics[width=0.9\textwidth]{./sl-hold-out-sample.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgf952fcf}]{\large IPCW and pseudo-outcome require pre-specified censoring estimator}
\def\shift{3}
\def\ls{}
\def\lw{.5mm}
\centering \Large
...
\begin{frame}[label={sec:org7ea3ea3}]{The state learner: Model all `states' of the \emph{observed} data}
\vspace{-1cm}
...
\begin{frame}[label={sec:org091fd93}]{Building a library for \(F\)}
The formulas
\begin{align*}
  F(t, 0, x)
  &
  ...
  % = P{\left(
    ...
    % \right)}
    = \int_0^t F(t-,0, x)  \Lambda_{j}(\diff s \mid x),
    \quad  j \in \{1,2\},
  \\
  F(t, -1, x)
  &
  % =
    ...
    = \int_0^tF(t-,0, x)  \Gamma(\diff s \mid x),
\end{align*}
...
\begin{frame}[label={sec:org087695b}]{Illustration with data from a prostate cancer study\footnote{\cite{kattan2000pretreatment}.}}
\begin{onlyenv}<1>
...
\begin{onlyenv}<2>
The target parameter was the average treatment effect of hormone therapy.
...
\begin{frame}[label={sec:orge07070d}]{Additional details}
...
\begin{frame}[label={sec:org0e4de4e}]{References}
