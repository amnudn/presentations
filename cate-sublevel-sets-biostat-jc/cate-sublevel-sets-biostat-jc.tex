...
\begin{frame}[label={sec:org8b3a733}]{Motivation}
...
Objective to estimate lower-dimensional \color{bblue}summaries
\color{black} or \color{bblue}features \color{black} of the CATE
function which can be visualized and interpreted.
\end{frame}

\begin{frame}[label={sec:org24560bc}]{Formal setup}
...
\begin{frame}[label={sec:org16194de}]{Idea: Study the area of the sublevel sets of \(\tau\)}
\vspace{.5cm}

The (sub)level set of the function
$\tau(P) \colon \mathcal{W} \rightarrow [-1,1]$ for level
$\alpha \in [-1,1]$ is 
\begin{equation*}
...
\begin{frame}[label={sec:org02ea4f6}]{Estimand}
...
\begin{frame}[label={sec:org9a2487f}]{Illustration of the function \(\gamma(P) \colon [-1,1] \rightarrow [0,1]\)}
\hfill
...
\begin{frame}[label={sec:org8587504}]{Estimation}
...
\begin{frame}[label={sec:org006e6ec}]{Estimators}
...
\begin{frame}[label={sec:org3c7983e},fragile]{Naive plug-in (i.e., the obvious) estimator}
 \begin{onlyenv}<1>
...
\begin{frame}[label={sec:org7e2eac3}]{Generalized Grenander-type estimator\footnote{\cite{westling2020unified,van2006estimating,groeneboom1983density,grenander1956theory}}}
...
\begin{enumerate}
\item Estimate the primitive function (aka anti-derivative) \(\alpha
   \mapsto \Gamma(P)(\alpha) = \int_{-1}^{\alpha} \gamma(P)(u) \diff u\)
\item Find the greatest convex minorant of the estimator
\(\hat\Gamma_n\)
...
\begin{frame}[label={sec:org60a7845}]{Approximate estimand}
\vfill
...
\begin{frame}[label={sec:orgabe3dbd}]{Orthogonal statistical learning \citep{foster2023orthogonal}}
...
\begin{frame}[label={sec:orgb77e8d7}]{Challenges}
\vfill
...
\begin{frame}[label={sec:org4afa823}]{Next project}
...
\begin{frame}[label={sec:org5860268}]{References}
