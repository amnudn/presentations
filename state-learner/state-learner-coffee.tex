...
\label{sec:org12fea48}
\begin{frame}[label={sec:orge2c4cdc}]{Existing approaches}
\small
...
\label{sec:org07d093e}
\begin{frame}[label={sec:orgece9732}]{The observed multi-state system}
...
\begin{frame}[label={sec:org1b9c634}]{Conditional state-occupation probabilities for observed data}
\small
...
\begin{frame}[label={sec:orgdde6f8e}]{The state learner}
\small
...
\begin{frame}[label={sec:org662c453}]{Expressing \(F\) using \(\Lambda\) and \(\Gamma\)}
\small
...
\begin{frame}[label={sec:org781cf60},fragile]{Almost minimum viable product}
 \footnotesize
